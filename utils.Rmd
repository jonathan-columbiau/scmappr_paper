...
```{r}
CreateAllModels <- function(reference_dataset, celltype_labels) {
  ...
  function_list <- c(pairwise_linear_svm,pairwise_polynomial_svm,pairwise_naive_bayes,pairwise_ridge,pairwise_lasso,pairwise_elastic_net,pairwise_lda,pairwise_knn,pairwise_random_forest,pairwise_qda)
...
LDA Algorithm
```{r}
pairwise_lda <- function(reference_dataset, celltype_labels) {
...
```{r}
#from MASS package
pairwise_qda <- function(reference_dataset, celltype_labels) {
    #upsample minority class to make class frequencies equal
    reference_dataset <- upSample(x = reference_dataset, y = celltype_labels, yname = "celltype_labels")
...
```{r}
pairwise_naive_bayes <- function(reference_dataset, celltype_labels) {
    #upsample minority class to make class frequencies equal
    reference_dataset <- upSample(x = reference_dataset, y = celltype_labels, yname = "celltype_labels")
...
```{r}
#e1071 package
pairwise_linear_svm <- function(reference_dataset, celltype_labels) {
    reference_dataset <- upSample(x = reference_dataset, y = celltype_labels, yname =  "celltype_labels")
...
```{r}
#e1071 package
pairwise_polynomial_svm <- function(reference_dataset, celltype_labels) {
    reference_dataset <- upSample(x = reference_dataset, y = celltype_labels, yname =  "celltype_labels")
...
```{r}
#glmnet package
pairwise_elastic_net <- function(reference_dataset, celltype_labels) {
...
```{r}
#glmnet package
pairwise_lasso <- function(reference_dataset, celltype_labels) {
    upsampled_dataset <- upSample(x = reference_dataset, y = celltype_labels, yname =  "celltype_labels", list = T)
    celltype_labels <- upsampled_dataset[["y"]]
    reference_dataset <- upsampled_dataset[["x"]] %>% as.matrix()
...
```{r}
#glmnet package
pairwise_ridge <- function(reference_dataset, celltype_labels) {
    upsampled_dataset <- upSample(x = reference_dataset, y = celltype_labels, yname =  "celltype_labels", list = T)
    celltype_labels <- upsampled_dataset[["y"]]
    reference_dataset <- upsampled_dataset[["x"]] %>% as.matrix()
...
```{r}
pairwise_knn <- function(reference_dataset, celltype_labels) {
  #upsample minority class to make class frequencies equal
    reference_dataset <- upSample(x = reference_dataset, y = celltype_labels, yname = "celltype_labels")
...
```{r}
GetModels <- function(marker_genes, ref_bpcells, ref_metadata, tree, metadata_cluster_column = "cluster_label", metadata_cell_label_column = "cell_label", n_cells_sampled = 500) {
 #1) Normalize reference atlas.
  # Normalize by reads-per-cell
  ref_bpcells <- multiply_cols(ref_bpcells, 1/Matrix::colSums(ref_bpcells))
...
```{r}
Classify <- function(bpcells_query, models, tree_struc) {
  ...
  for (j in 1:length(internal_nodes)) { # iterate node
    ...
    count_threshold <- (child(tree_struc,node) %>% length() - 1) * 10 * .66 #2/3 of max score (num matchups for each class - 1)*10
